...
public class MainActivity4 extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		listview.setOnTouchListener(new View.OnTouchListener() {
			...
			public boolean onTouch(View v, MotionEvent event) {
				Util.hideKeyboard(MainActivity4.this);
	...
	public void delete(View v) {
		...
		//체크된 라디오 버튼의 인덱스 값을 얻어온다
		int chechedIndex = listview.getCheckedItemPosition();
		//모델에서 해당 인덱스의 데이터를 삭제한다.
		if(chechedIndex == -1) return;	//선택하지 않았다면 메소드 종료 하기
		list.remove(chechedIndex);
		//아답타에 모델의 데이터가 바뀌었다고 알린다.
		adapter.notifyDataSetChanged();
		//선택된 라디오 버튼 초기화(unselect)
